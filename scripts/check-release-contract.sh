...
if ! printf '%s\n' "$VERSION" \
  ...
  echo "usage: check-release-contract.sh <version> [notes-file]" >&2
  exit 2
fi
...
if versions != {version}:
    raise SystemExit(f"release version mismatch: expected {version}, got {sorted(versions)}")
