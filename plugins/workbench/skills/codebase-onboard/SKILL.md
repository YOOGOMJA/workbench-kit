...
### 3. clone — workbench setup

작업 공간에서 `workbench setup`을 실행한다. 매니페스트를 읽어
...
- 이로써 `workbench task add-repo <name>`의 전제(`.codebases/<name>` 존재)가 충족된다.

### 5. role 안내 (해당 시)

이 repo를 *수정 대상*이 아니라 *참고용*으로만 쓸 거면, 나중에 task에 붙일 때
`workbench task add-repo <name> --role reference`로 단다(제출·PR 대상에서 제외). 지금
...
- **배관을 바꾸지 않는다.** `workbench setup`·`codebases.yaml` 형식·
...
4. `workbench setup` → `.codebases/timetree-extractor` clone.
